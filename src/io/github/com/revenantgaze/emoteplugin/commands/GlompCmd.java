...
public class GlompCmd implements CommandExecutor {
	
	public Main plugin;
	
	public GlompCmd(Main instance) {
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (cmd.getName().equalsIgnoreCase("glomp")) {
			...
			if (Cooldown.tryCooldown(you, "MagicHand", (CooldownValue * 1000))) {
				...
				if ((args.length == 1) && (Bukkit.getPlayerExact(args[0]) != null)){
					...
					String user2 = target.getName();
						...
						Bukkit.getServer().broadcastMessage(ChatColor.GOLD + "[Emotes] " + ChatColor.GREEN + user1 + " glomps " + user2 + "! It's Super effective!");
					...
					String user1 = you.getName();
						...
						Bukkit.getServer().broadcastMessage(ChatColor.GOLD + "[Emotes] " + ChatColor.GREEN + user1 + " glomps a block of cobblestone! It's not very effective");
