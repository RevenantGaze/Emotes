			...
			if (Cooldown.tryCooldown(you, "MagicHand", (CooldownValue * 1000))) {
				...
				if ((args.length == 1) && (Bukkit.getPlayerExact(args[0]) != null)){
					...
					String user2 = target.getName();
						...
						Bukkit.getServer().broadcastMessage(ChatColor.GOLD + "[Emotes] " + ChatColor.GREEN + user1 + " winks at " + user2 + "! ;)");
					...
					String user1 = you.getName();
						...
						Bukkit.getServer().broadcastMessage(ChatColor.GOLD + "[Emotes] " + ChatColor.GREEN + user1 + " winks to a creeper! ;)");
